 ...
 *
 * Usage:
 *   node scripts/generate-all.mjs --company="Acme" --override=<path> [options]
 *
 * Options:
...
import { showDataPreview, validateData } from '../lib/typst-util.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));

// Import resume generator functions
...
// Import cover generator functions
...
async function main() {
  ...
  for (const arg of args) {
    ...
    if (arg.startsWith('--output-dir=')) { outputDir = arg.split('=')[1]; continue; }
  ...
  if (!company) {
    console.error(`
Usage: node scripts/generate-all.mjs --company="Acme" [options]
...
`);
  ...
  const resumePath = resolve(outputDir, `Fabricio-Pirini-${companySlug}-Resume.pdf`);
  const coverPath = resolve(outputDir, `Fabricio-Pirini-${companySlug}-Cover-Letter.pdf`);

  const flags = { watch, dryRun, validate: validateOnly };

  log.i(`Generating for: ${company}`);

  // Generate resume
  log.i('--- RESUME ---');
  await generateResume(resumePath, variant, overridePath, flags);

  // Generate cover letter (if body provided)
  if (body || watch) {
    log.i('--- COVER LETTER ---');
